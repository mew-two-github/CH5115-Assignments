...
Bc = [1/A1 ; 0];
Cc = [0,Cv2]; % If only Fo,2 is sensed
...
% it is possible to estimate the states.(since, in this model, the states
...
subplot(2,1,1)
stairs(T,Y);title('System Response');xlabel('Time');
ylabel('Outlet flow rate F_{o,2}');
subplot(2,1,2);
stairs(T,ut);ylim([-1.5 1.5]);xlabel('Time');ylabel('Input move');
%% Part-e)
% Declare the model in the form required by Kalman function
G = [sqrt(Q(1,1));sqrt(Q(2,2))]; H = 0; D = 0;
...
sim2 = feedback(sim,1,4,2,1); 
%I am not sure what the 1 at the beginning correspond to(I copy pasted from the kalmfiltdemo
%posted on moodle). It works for scalar Q but not for matrix Q. I understand 4 and 2
%correspond to the input and output ports and 1 means positive gain.
...
var_xkhat = sum(var(outmat(:,4:5) - x(:,1:2)));
var_ykhat = var(outmat(:,3) - outmat(:,2));
